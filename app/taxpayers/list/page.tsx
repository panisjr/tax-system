  ...
  return (
    <div className="w-full">
      ...
      <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-lexend text-2xl font-bold text-[#595a5d]">Taxpayer Master List</h1>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
        {[
          { label: 'Total Taxpayers', value: isLoading ? '—' : taxpayers.length.toLocaleString(), color: 'text-[#595a5d]' },
          { label: 'Individual',      value: isLoading ? '—' : taxpayers.filter((t) => t.owner_type === 'Individual').length.toLocaleString(),  color: 'text-blue-600' },
          { label: 'Corporate',       value: isLoading ? '—' : taxpayers.filter((t) => t.owner_type === 'Corporate').length.toLocaleString(),   color: 'text-amber-600' },
          { label: 'Government',      value: isLoading ? '—' : taxpayers.filter((t) => t.owner_type === 'Government').length.toLocaleString(),  color: 'text-emerald-600' },
        ].map((s) => (
          ...
          <table className="w-full font-inter text-xs">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                {['#', 'Full Name', 'TIN', 'Type', 'Address', 'Phone', 'Email', 'Actions'].map((h) => (
                  <th
                    key={h}
                    className="whitespace-nowrap px-4 py-3 text-left text-[#595a5d] font-semibold uppercase tracking-wide"
                  >
                    {h}
                  </th>
                ))}
                      ...
                      {t.owner_type ? (
                        <span
                          className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                            ownerTypeColor[t.owner_type] ?? 'bg-gray-100 text-gray-600'
                          }`}
                        >
                          {t.owner_type}
                        </span>
